...
def wikiConverse(userInput):
    stripWhiteSpace = userInput.strip().lower()
    toWiki = ""
    if "search for" in stripWhiteSpace:
        toWiki = stripWhiteSpace.replace("search for", "")
    else :
        toWiki = stripWhiteSpace.replace("tell me about", "")
    wiki = wikipedia.page(toWiki.strip())
    reply = wikipedia.summary(toWiki, sentences=2)
